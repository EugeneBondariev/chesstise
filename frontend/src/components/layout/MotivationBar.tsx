...
export default function MotivationBar() {
	...
	useEffect(() => {
		...
		const priorDates = Object.keys(goalHoursByDate)
			.filter((d) => d < todayKey)
			.sort()
		const lastGoal = priorDates.length
			? goalHoursByDate[priorDates[priorDates.length - 1]]
			: 0
	...
	const goalData = useMemo(() => {
		...
		const runningMs = running
			? Math.max(0, now.getTime() - running.startedAt)
			: 0
		...
		if (
			...
			try {
				...
				new Notification('Chesstíse — behind schedule', {
					body: `${formatHM(actualMs)} / ${formatHM(
						goalHours * 3_600_000
					)} done`,
				})
			} catch {
	...
	const runningMsDisplay = running
		? Math.max(0, now.getTime() - running.startedAt)
		: 0
	...
	const handleAddManual = () => {
		const ms =
			(Math.max(0, manualH) * 60 + Math.max(0, Math.min(59, manualM))) * 60_000
		...
		<div className='motivation-bar'>
			<div className='motivation-controls'>
				...
				<span className='motivation-total' aria-live='off'>
					{formatHM(actualMs)}
					{hasGoal ? ` / ${formatHM(goalHours * 3_600_000)}` : ''}
				</span>

				{historyRows.length > 0 && (
					<button
						className='motivation-history-btn'
				...
				<div className='motivation-manual-wrap' ref={manualRef}>
					...
					{showManual && (
						<div
							className='motivation-manual-popover'
							role='dialog'
						...
						>
							<div className='motivation-manual-row'>
								...
								m
								<button
									className='motivation-manual-add'
							...
							{manualMsToday > 0 && (
								<div className='motivation-manual-existing'>
									...
									<button
										...
										onClick={() => clearManualMs(todayKey)}
									>
										Clear
									</button>
								</div>
			...
			<div className='motivation-timeline-wrap'>
				<div className={`motivation-timeline${isLate ? ' late' : ''}`}>
					<div className='motivation-timeline-track'>
						{hasGoal && (
							<div
								className='motivation-timeline-fill'
						...
						{TICK_HOURS.map((h) => (
							<div
								key={h}
								className='motivation-timeline-tick'
								style={{ left: `${hourPct(h)}%` }}
							/>
						))}
						<div
							className='motivation-timeline-now'
				...
				<span className={`motivation-status${isLate ? ' late' : ''}`}>
					{!hasGoal
						...
						: isLate
						? `Behind schedule — ${formatHM(
								goalData.expected * 3_600_000 - actualMs
						  )} to catch up`
						: freeUntilTime
								...
								<table>
									...
									<tbody>
										{historyRows.map(({ date, goalH: gh, doneMs }) => {
											const met =
												gh > 0 &&
												Math.floor(doneMs / 60_000) >= Math.round(gh * 60)
											const behind = gh > 0 && !met
											return (
												<tr
													key={date}
													className={
														date === todayKey ? 'motivation-history-today' : ''
													}
												>
													...
													<td
														className={
															met ? 'hist-met' : behind ? 'hist-behind' : ''
														}
													>
														{met ? '✓' : behind ? '✗' : ''}
													</td>
					...
					>
						...
						>
							...
							<div className='motivation-alert-body'>
								<p style={{ margin: 0 }}>
									{formatHM(actualMs)} of {formatHM(goalHours * 3_600_000)}{' '}
									done. Time to grind.
								</p>
							</div>
